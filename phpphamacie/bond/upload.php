...
if(isset($_FILES['image'])){
    ...
    if ($uploadOk == 0) {
        ...
        if (move_uploaded_file($_FILES["image"]["tmp_name"], $target_file)) {
           ...
           // $date = date("Y-m-d");
            $sql = "INSERT INTO bonphamacie (nomimage , chemin,iduser,datecommade) VALUES ('". basename($_FILES["image"]["name"]) . "', '" . $target_file . "','".$_SESSION["id"]."','".$date."')";
    ...
    if(isset($_FILES['images'])){
        ...
        if (move_uploaded_file($_FILES["images"]["tmp_name"], $target_file)) {
            ...
            $sql = "SELECT chemin FROM bonphamacie WHERE id = $idimage";
            ...
            $sql = "UPDATE bonphamacie SET chemin = '$target_file' WHERE id = $idimage";
